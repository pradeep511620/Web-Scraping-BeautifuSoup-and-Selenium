...
myresult=['https://www.greenlee.com/sitemaps/www_greenlee_com/us/en/productssitemap-001.xml']
...
for row in myresult:
 ...
 try:
    ...
    while i < len(model):
            ...
            save_details: TextIO = open("greenlee-check.txt", "a+", encoding="utf-8")
            save_details.write("\n"+c+"\t"+a+"\t"+y+"\t"+" > ".join(list1)+"\t"+" , ".join(list2)+"\t"+sku11+"\t"+sku22+"\t"+"RP_"+z+"\t"+"RP_"+x)
            save_details.close()
