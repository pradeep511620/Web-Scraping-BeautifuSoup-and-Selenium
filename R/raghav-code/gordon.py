...
def get_details(url):
    ...
    save_details: TextIO = open("sqd1.txt", "a+")
    ...
    try:
        for i in range(1,61):
            ...
            try:
                ...
                pass
            save_details: TextIO = open("sqd1.txt", "a+", encoding="utf-8")
            save_details.write("\n" + ele + "\t" + product_name1 + "\t" + upc  +"\t"+ bc + "\t"+image+ "\t" +table1)
            save_details.close()
...
if __name__ == '__main__':
    for x in range(153,160):
     x=str(x)
     url = "https://www.gordonelectricsupply.com/s/Square-D-Co/Square-D-Co/mfr-1IIX?process=search&itemsperpage=60&sortby=availSort&display=thumb&fqall=&fqv=&fq=&sf=&allnstr=&ph=&pl=&qdx=0&nstr=&narrowby=&pagenum="+x
     x=int(x)
     print(url)
