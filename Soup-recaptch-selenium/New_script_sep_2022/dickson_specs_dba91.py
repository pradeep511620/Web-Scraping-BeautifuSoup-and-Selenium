...
cur = mydb.cursor()
cur.execute("SELECT url,attribute_name,attribute_value,id FROM `dickson_specs_dba91` where attr_test is null ")
...
for fetch in myresult:
    ...
    if flag == 1:
        par = ("attribute found")
        sql = ("UPDATE `dickson_specs_dba91` SET attr_test ='" + str(par) + "' WHERE   id='" + str(id) + "'")
        cur.execute(sql)
        mydb.commit()
        ...
        sql = ("UPDATE `dickson_specs_dba91` SET attr_test ='" + str(par1) + "' WHERE   id='" + str(id) + "' ")
        cur.execute(sql)
        mydb.commit()
        print(cur.rowcount, "records successful Done")
