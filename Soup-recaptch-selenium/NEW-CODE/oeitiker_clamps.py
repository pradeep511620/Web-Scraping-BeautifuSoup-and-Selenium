...
cur = mydb.cursor()
cur.execute("SELECT product_url,attr_name,attr_value,id FROM `oetiker_clamps_attr`  ")
...
for fetch in myresult:
    ...
    if(flag == 1):
        par = ("attribute found")
        sql = ("UPDATE `oetiker_clamps_attr` SET b2b_attr_test ='" + str(par) + "' WHERE   id='" + str(id)+"' ")
        cur.execute(sql)
        mydb.commit()
        ...
        sql = ("UPDATE `oetiker_clamps_attr` SET b2b_attr_test ='" + str(par1) + "' WHERE   id='" + str(id)+"' ")
        cur.execute(sql)
        mydb.commit()
        print(cur.rowcount, "records successful Done")
