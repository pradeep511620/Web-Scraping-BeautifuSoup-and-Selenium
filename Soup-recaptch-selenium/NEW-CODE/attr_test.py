...
cur = mydb.cursor()
cur.execute("SELECT product_url,attr_name,attr_value,id FROM `genral_pipe_cleaner_attr` ")
...
for fetch in myresult:
    ...
    if flag == 1:
        par = ("attribute found")
        sql = ("UPDATE `genral_pipe_cleaner_attr`SET attr_test ='" + str(par) + "' WHERE   id='" + str(id) + "'")
        cur.execute(sql)
        mydb.commit()
        ...
        sql = ("UPDATE `genral_pipe_cleaner_attr`SET attr_test ='" + str(par1) + "' WHERE   id='" + str(id) + "' ")
        cur.execute(sql)
        mydb.commit()
        print(cur.rowcount, "records successful Done")
