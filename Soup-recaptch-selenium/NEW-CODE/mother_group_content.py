...
cur=mydb.cursor()
cur.execute('select product_url,mother_desc,meta_title,meta_desc,product_title from `mother_content_aug`')
...
for fetch in myresult:
    ...
    for desc in soup.find_all('p',class_='comment more'):
        d=str(desc.text)
        t1 = description.split('\n')
        t2 = "".join(t1)
        if(d.split()==t2.split()):
            desc1=('desc is same')
            sql = ("UPDATE `mother_content_aug`  SET desc_test ='" + str(desc1) + "' WHERE   product_url='" + str(product_url) + "'")
            cur.execute(sql)
            mydb.commit()
            print(cur.rowcount, "records successful Done")
        else:
            desc2=('desc is not same')
            sql = ("UPDATE `mother_content_aug` SET desc_test ='" + str(desc2) + "' WHERE   product_url='" + str(
                product_url) + "'")
            cur.execute(sql)
            mydb.commit()
            print(cur.rowcount, "records successful Done")
