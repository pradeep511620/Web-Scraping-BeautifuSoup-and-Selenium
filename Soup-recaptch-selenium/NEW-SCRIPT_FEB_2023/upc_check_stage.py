...
cur = mydb.cursor()
cur.execute("SELECT product_url,attribute_value,id FROM `dba_221_upc` WHERE upc_test is null")
myresult = cur.fetchall()
for fetch in myresult:
    product_url = fetch[0]
    upc = fetch[1].replace('rp_', '').replace('RP_','').replace('upc:','')
    id = fetch[2]
    ...
    if ship != "not given":
        ...
        if flag == 1:
            par = ("attribute found")
            sql = ("UPDATE `dba_221_upc` SET upc_test ='" + str(par) + "' WHERE   id='" + str(id) + "'")
            cur.execute(sql)
            mydb.commit()
            ...
            sql = ("UPDATE `dba_221_upc` SET upc_test ='" + str(par1) + "' WHERE   id='" + str(id) + "'")
            cur.execute(sql)
            mydb.commit()
            print(cur.rowcount, "records successful Done")
