...
cur = mydb.cursor()
cur.execute("SELECT url FROM `sem_req` where remarks is null")
...
for fetch in my_result:
    ...
    if ship != "not given":
        if soup.find("div", class_="priceInnerwrap").find_all('span'):
            ...
            for pd in soup.find("div", class_="priceInnerwrap").find_all('span'):
                ...
                if a == 1:
                    ...
                    if pri2 <=300.00:
                        prod1 = 'less 300 '
                        sql = ("UPDATE `sem_req` SET remarks_less_300 ='" + str(
                            prod1) + "' WHERE   url='" + str(
                            product_url) + "'")
                        cur.execute(sql)
                        mydb.commit()
        ...
        sql = ("UPDATE `sem_req` SET remarks_ship_blank ='" + str(
            prod1) + "' WHERE   url='" + str(
            product_url) + "'")
        cur.execute(sql)
        mydb.commit()
        print(cur.rowcount, "records successful Done")
    y ='yes'
    sql = ("UPDATE `sem_req` SET remarks ='" + str(
        y) + "' WHERE   url='" + str(
        product_url) + "'")
    cur.execute(sql)
    mydb.commit()
