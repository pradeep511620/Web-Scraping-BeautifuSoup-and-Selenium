...
cur = mydb.cursor()
cur.execute("SELECT product_url,l3_name,parent_name,item_name,product_title,model_no,brand FROM `dba_187_arrow` where product_name_test is null ")
...
for fetch in myresult:
    ...
    for pd in soup.find("div", class_="about-product").find_all("h1"):
        ...
        if pro.lower().strip() == product_title.lower().strip():
            ...
            sql = ("UPDATE `dba_187_arrow`SET product_name_test ='" + str(prod) + "' WHERE   product_url='" + str(
                product_url) + "'")
            cur.execute(sql)
            mydb.commit()
            ...
            sql = ("UPDATE `dba_187_arrow`SET product_name_test ='" + str(prod1) + "' WHERE   product_url='" + str(
                product_url) + "'")
            cur.execute(sql)
            mydb.commit()
            print(cur.rowcount, "records successful Done")
        ...
        for item in soup.find("div", class_="accordion-div").find_all("td"):
            ...
            if b == 1:
                ...
                if (item_name.lower().strip() == it1.lower().strip()):
                    ...
                    sql = ("UPDATE `dba_187_arrow`SET item_test ='" + str(item1) + "' WHERE   product_url='" + str(
                        product_url) + "'")
                    cur.execute(sql)
                    mydb.commit()
                    print(cur.rowcount, "records successful Done")
                    ...
                    sql = ("UPDATE `dba_187_arrow`SET item_test ='" + str(item2) + "' WHERE   product_url='" + str(
                        product_url) + "'")
                    cur.execute(sql)
                    mydb.commit()
                    print(cur.rowcount, "records successful Done")
                ...
                for bread in soup.find("ul", class_="items breadcrumb").find_all("span"):
                    ...
                    if (a == 10):
                        ...
                        if (l3.lower().strip() == l3_name.lower().strip()):
                            l3_n = ("l3_name same")
                            sql = ("UPDATE `dba_187_arrow`SET l3_test ='" + str(
                        ...
                        if par.lower().strip() == parent_name.lower().strip():
                            ...
                            sql = ("UPDATE `dba_187_arrow`SET parent_test ='" + str(
