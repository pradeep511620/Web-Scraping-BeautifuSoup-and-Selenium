...
mydb = mysql.connector.connect(
    ...
    user="root",
    password="",
    database="new_data_2022_feb"
)
cur = mydb.cursor()
cur.execute("SELECT product_url,l3_name,parent_name,item_name FROM air_master_1436 where parent_test = 'parent_name not same' ")
    ...
    for item in soup.find("div", class_="productDetailsLeft half-collomn").find_all("td"):
        ...
        if (b == 2):
            ...
            if (item_name.lower().strip() == it1.lower().strip()):
                ...
                sql = ("UPDATE air_master_1436 SET item_test ='" + str(item1) + "' WHERE   product_url='" + str(
                    product_url) + "'")
                cur.execute(sql)
                mydb.commit()
                ...
                sql = ("UPDATE air_master_1436 SET item_test ='" + str(item2) + "' WHERE   product_url='" + str(
                    product_url) + "'")
                cur.execute(sql)
                mydb.commit()
                print(cur.rowcount, "records successful Done")
            ...
            for bread in soup.find("ul", class_="items breadcrumb").find_all("span"):
                ...
                if a == 10:
                    ...
                    if (l3.lower().strip() == l3_name.lower().strip()):
                        l3_n = ("l3_name same")
                        sql = ("UPDATE air_master_1436 SET l3_test ='" + str(
                    ...
                    if par.lower().strip() == parent_name.lower().strip():
                        ...
                        sql = ("UPDATE air_master_1436 SET parent_test ='" + str(
