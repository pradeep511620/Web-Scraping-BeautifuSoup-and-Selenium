...
cur = mydb.cursor()
cur.execute("SELECT product_url,attr_name,attr_value,id FROM `watts_specs_dba_13` where attr_test='attribute not found'")
...
for fetch in myresult:
    ...
    if flag == 1:
        par = "attribute found"
        sql = ("UPDATE `watts_specs_dba_13` SET attr_test ='" + str(par) + "' WHERE   id='" + str(id) + "'")
        cur.execute(sql)
        mydb.commit()
        ...
        sql = ("UPDATE `watts_specs_dba_13` SET attr_test ='" + str(par1) + "' WHERE   id='" + str(id) + "' ")
        cur.execute(sql)
        mydb.commit()
        print(cur.rowcount, "records successful Done")
