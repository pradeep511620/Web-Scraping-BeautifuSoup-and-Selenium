...
cur = mydb.cursor()
cur.execute("SELECT product_url,price FROM `krytox_price_dba_9` where price_test is null")
...
for fetch in my_result:
    ...
    for pd in soup.find("div", class_="priceInnerwrap").find_all('span'):
        ...
        if a == 1:
            ...
            if rs == price1:
                pri = "price same"
                sql = ("UPDATE `krytox_price_dba_9` SET price_test ='" + str(pri) + "',product_price='" + str(
                ...
                pri1 = "price not same"
                sql = ("UPDATE `krytox_price_dba_9` SET price_test ='" + str(pri1) + "',product_price='" + str(
                    rs) + "',calculate_price='" + str(price1) + "' WHERE   product_url='" + str(product_url) + "'")
                cur.execute(sql)
                mydb.commit()
                print(cur.rowcount, "records successful Done")
