...
cur = mydb.cursor()
cur.execute("SELECT product_url,attribute_name,attribute_value FROM attr_americraft_com where attr_test is null")
    ...
    for attr in soup.find("div",class_="col-sm-6 productdetailleft").find_all("td"):
        ...
        if(a==4) or (a==6) or (a==8) or (a==10) or (a==12) or (a==14) or (a==16):
            ...
            if(attribute_value.lower().strip()==blade.lower().strip()):
                ...
                sql = ("UPDATE attr_americraft_com SET attr_test ='" + str(attr1) + "' WHERE   product_url='" + str(product_url) + "'")
                cur.execute(sql)
                mydb.commit()
