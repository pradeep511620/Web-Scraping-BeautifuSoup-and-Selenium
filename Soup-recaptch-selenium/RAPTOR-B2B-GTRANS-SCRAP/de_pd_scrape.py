...
cur = mydb.cursor()
cur.execute("SELECT url,id FROM `z`  WHERE scrap_data_de IS NULL and scrap_data is not null ")
...
for fetch in my_result:
    url = fetch[0]
    pd_url = url.replace('//b2b.', '//www.').replace('.com/', '.de/')
    id = fetch[1]
    ...
    try:
        ...
        if result.status_code == 200 and result.history !=301:
            print("\n site working")
            working += 1
        else:
        ...
        while i < len(prod_i):
            ...
            while j < len(spec_i):
                ...
                sql = "INSERT INTO de_pd_specs(product_url,brand,model_no,attribute_name,attribute_value) VALUES (%s, %s ,%s ,%s, %s)"
                val = (product_url, brand, modelNo, z, x)
                cur.execute(sql, val)
                mydb.commit()
                print(cur.rowcount, "record(s) affected")

        sql = "INSERT INTO de_pd_detail(product_url,brand,product_title,item_id,model_no,pd_detail) VALUES (%s ,%s ,%s, %s,%s,%s)"
        val = (product_url, brand, title, itemId, modelNo, desc)
        cur.execute(sql, val)
        mydb.commit()
        print(cur.rowcount, "record(s) affected")
        print(id)
        sql = ("UPDATE `z` SET scrap_data_de ='yes' WHERE   id='" + str(id) + "'")
        cur.execute(sql)
        mydb.commit()
