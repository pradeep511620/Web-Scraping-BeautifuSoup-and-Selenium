...
cur = mydb.cursor()
cur.execute("SELECT pd_url,id FROM  `raptor_scrape_google_feed`.`pl_url` where status is null ")
    ...
    try:
        ...
        while i < len(prod_i):
            ...
            while j < len(spec_i):
                ...
                sql = "INSERT INTO `raptor_scrape_google_feed`.pl_pd_specs(product_url,brand,model_no,attribute_name,attribute_value,attr_count) VALUES (%s, %s ,%s ,%s, %s,%s)"
                val = (product_url, brand, modelNo, z, x, i)
                cur.execute(sql, val)
                mydb.commit()
                print(cur.rowcount, "record(s) affected")

        sql = "INSERT INTO `raptor_scrape_google_feed`.pl_pd_detail(product_url,brand,product_title,item_id,model_no,cross_ref,breadcrum,pd_detail) VALUES (%s, %s ,%s ,%s, %s,%s,%s,%s)"
        val = (product_url, brand, title, itemId, modelNo,crosRef,b,desc)
        cur.execute(sql, val)
        mydb.commit()
        print(cur.rowcount, "record(s) affected")
        sql = ("UPDATE `raptor_scrape_google_feed`.`pl_url` SET status ='yes' WHERE   id='" + str(id) + "'")
        cur.execute(sql)
        mydb.commit()
