...
cur.execute("SELECT  new_attr_name,new_attr_value,model_no,entity_id,l3_name,id,value1,unit1,value2,unit2,brand,"
            ...
            " remarks_25='missing values'  AND attr_data_type ='Continuous' AND value1 ='' AND new_attr_name='Thread Size' AND new_attr_value LIKE '%-%' ")
...
for fetch in myresult:
    ...
    if new_attr_name == 'Thread Size':
        s11 = new_attr_value.split('-')[0]
        s1 = new_attr_value.split('-')[1]
        s2 = "rp_" + s1
        s = s11 + '"'
        z = 'Thread Dia.'
        z1 = 'Thread Per Inch'
        ...
        sql = "INSERT INTO `58_l3_update_new_value` (new_attr_value,new_attr_name,entity_id,model_no,l3_name,value1," \
              ...
              "%s,%s,%s,%s,%s,%s,%s,%s,%s) "
        val = (s, z, entity_id, model_no, l3_name, s11, dc, value2, unit2, brand, raw_value, attr_data_type,
        ...
        sql = "INSERT INTO `58_l3_update_new_value` (new_attr_value,new_attr_name,entity_id,model_no,l3_name,value1," \
              "unit1,value2,unit2,brand,raw_value,attr_data_type,old_attr_value,grainger) VALUES (%s, %s ,%s ,%s, %s," \
              "%s,%s,%s,%s,%s,%s,%s,%s,%s) "
        val = (s2, z1, entity_id, model_no, l3_name, s2, unit1, value2, unit2, brand, raw_value, attr_data_type,
               old_attr_value, grainger)
        cur.execute(sql, val)
        mydb.commit()
        print(cur.rowcount, "record(s) affected")
