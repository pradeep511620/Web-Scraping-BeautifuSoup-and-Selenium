...
cur = mydb.cursor()
cur.execute("SELECT  attr_name,attr_value,brand,model_no,id FROM `helicoil_chrislynn_split`  WHERE attr_value LIKE '%#%' AND attr_name !='Drill Size' ")
...
for fetch in myresult:
    ...
    if attr_name :
        s11 = attr_value.split(' - ')[0]
        s1 = attr_value.split(' - ')[1]
        s2 = "rp_" + s1
        s = "rp_" + s11
        z = 'Thread Dia.'
        z1 = 'Thread Per Inch'
