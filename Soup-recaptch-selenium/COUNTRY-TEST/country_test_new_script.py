...
cur = mydb.cursor()
cur.execute("SELECT product_url,country,id FROM `pass_symour_oct_data`  where  country_test is null ")
...
try:
    for fetch in myresult:
        ...
        if(flag == 1):
            ...
            sql = ("UPDATE `pass_symour_oct_data` SET country_test ='" + str(par) + "' WHERE   id='" + str(id)+"'")
            cur.execute(sql)
            mydb.commit()
            ...
            sql = ("UPDATE `pass_symour_oct_data` SET country_test ='" + str(par1) + "' WHERE   id='" + str(id)+"' ")
            cur.execute(sql)
            mydb.commit()
            print(cur.rowcount, "records successful Done")
