...
cur = mydb.cursor()
cur.execute("SELECT product_url,image_name FROM `fasterner_image_dba82` where image_name !='' and image_test is null   ")
...
for fetch in myresult:
    ...
    for img in soup.find("div", class_='producImg').find_all('a'):
        ...
        if status == 200:
            ...
            if img1 == image_m:
                par = ('same')
                sql = ("UPDATE `fasterner_image_dba82`    SET image_test ='" + str(
