...
cur.execute("SELECT mother_url,short_desc,feature,working_mec,desc_asc,standard,id FROM `mother_content_dba92` where id>30  ")
    ...
    for x in soup.find("div", class_='mother_newsection').find_all('h4'):
        ...
        if y == "Working Mechanism":
            ...
            if c2.strip() == working_mec.strip():
                ...
                sql = ("UPDATE `mother_content_dba92` SET working_mec_test ='" + str(c3) + "' WHERE   id='" + str(
                    id) + "'")
                cur.execute(sql)
                mydb.commit()
                ...
                sql = ("UPDATE `mother_content_dba92` SET working_mec_test ='" + str(
                    c4) + "' WHERE   id='" + str(
                    id) + "'")
                cur.execute(sql)
                mydb.commit()
            ...
            if w2.strip() == feature.strip():
                f3 = 'feature same'
                sql = ("UPDATE `mother_content_dba92` SET feature_test ='" + str(
                    f3) + "' WHERE   id='" + str(
                    id) + "'")
                cur.execute(sql)
                mydb.commit()
            ...
            if co2.strip() == compatible_acc.strip():
                ...
                sql = ("UPDATE `mother_content_dba92` SET comp_acc_test ='" + str(
                    com1) + "' WHERE   id='" + str(
                    id) + "'")
                cur.execute(sql)
                mydb.commit()
            ...
            if s2.strip() == standard.strip():
                st1 = 'standard  same'
                sql = ("UPDATE `mother_content_dba92` SET standard_test ='" + str(
                    st1) + "' WHERE   id='" + str(
                    id) + "'")
                cur.execute(sql)
                mydb.commit()
