...
cur = mydb.cursor()
cur.execute("SELECT product_url,attr_name,attr_value,id FROM `real_tite_specs_dba63` where attr_test is null ")
...
for fetch in myresult:
    ...
    if flag == 1:
        par = ("attribute found")
        sql = ("UPDATE `real_tite_specs_dba63` SET attr_test ='" + str(par) + "' WHERE   id='" + str(id) + "'")
        cur.execute(sql)
        mydb.commit()
        ...
        sql = ("UPDATE `real_tite_specs_dba63` SET attr_test ='" + str(par1) + "' WHERE   id='" + str(id) + "' ")
        cur.execute(sql)
        mydb.commit()
        print(cur.rowcount, "records successful Done")
