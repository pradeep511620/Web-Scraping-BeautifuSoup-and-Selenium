...
cur = mydb.cursor()
cur.execute("SELECT product_url,l3_name,parent_name,item_name,product_title,model_no,brand FROM `baldor_sheet_dba61` where id>388 ")
...
for fetch in myresult:
    ...
    for pd in soup.find("div", class_="productInfoRight half-collomn").find_all("h1"):
        ...
        if pro.lower().strip() == product_title.lower().strip():
            ...
            sql = ("UPDATE `baldor_sheet_dba61` SET product_name_test ='" + str(prod) + "' WHERE   product_url='" + str(
                product_url) + "'")
            cur.execute(sql)
            mydb.commit()
            ...
            sql = ("UPDATE `baldor_sheet_dba61` SET product_name_test ='" + str(prod1) + "' WHERE   product_url='" + str(
                product_url) + "'")
            cur.execute(sql)
            mydb.commit()
            print(cur.rowcount, "records successful Done")
        ...
        for item in soup.find("div", class_="productDetailsLeft half-collomn").find_all("td"):
            ...
            if b == 2:
                ...
                if (item_name.lower().strip() == it1.lower().strip()):
                    ...
                    sql = ("UPDATE `baldor_sheet_dba61` SET item_test ='" + str(item1) + "' WHERE   product_url='" + str(
                        product_url) + "'")
                    cur.execute(sql)
                    mydb.commit()
                    print(cur.rowcount, "records successful Done")
                    ...
                    sql = ("UPDATE `baldor_sheet_dba61` SET item_test ='" + str(item2) + "' WHERE   product_url='" + str(
                        product_url) + "'")
                    cur.execute(sql)
                    mydb.commit()
                    print(cur.rowcount, "records successful Done")
                ...
                for bread in soup.find("ul", class_="items breadcrumb").find_all("span"):
                    ...
                    if (a == 10):
                        ...
                        if (l3.lower().strip() == l3_name.lower().strip()):
                            l3_n = ("l3_name same")
                            sql = ("UPDATE `baldor_sheet_dba61` SET l3_test ='" + str(
                        ...
                        if par.lower().strip() == parent_name.lower().strip():
                            ...
                            sql = ("UPDATE `baldor_sheet_dba61` SET parent_test ='" + str(
