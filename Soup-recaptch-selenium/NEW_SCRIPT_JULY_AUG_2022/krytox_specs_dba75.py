...
cur = mydb.cursor()
cur.execute("SELECT product_url,attr_name,attr_value,id FROM `krytox_specs_dba75`  ")
...
for fetch in myresult:
    ...
    if flag == 1:
        par = ("attribute found")
        sql = ("UPDATE `krytox_specs_dba75`  SET attr_test ='" + str(par) + "' WHERE   id='" + str(id) + "'")
        cur.execute(sql)
        mydb.commit()
        ...
        sql = ("UPDATE `krytox_specs_dba75`  SET attr_test ='" + str(par1) + "' WHERE   id='" + str(id) + "' ")
        cur.execute(sql)
        mydb.commit()
        print(cur.rowcount, "records successful Done")
