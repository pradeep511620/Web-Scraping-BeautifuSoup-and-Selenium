...
cur = mydb.cursor()
cur.execute("SELECT product_url,com_price,id FROM `price_dba78` where price_test is null  ")
...
for fetch in my_result:
    ...
    for pd in soup.find("div", class_="priceInnerwrap").find_all('span'):
        ...
        if a == 1:
            ...
            if rs == price1:
                pri = "price same"
                sql = ("UPDATE `price_dba78` SET price_test ='" + str(pri) + "',product_price='" + str(
                ...
                pri1 = "price not same"
                sql = ("UPDATE `price_dba78` SET price_test ='" + str(pri1) + "',product_price='" + str(
                    rs) + "',calculate_price='" + str(price1) + "' WHERE   id='" + str(id) + "'")
                cur.execute(sql)
                mydb.commit()
                print(cur.rowcount, "records successful Done")
