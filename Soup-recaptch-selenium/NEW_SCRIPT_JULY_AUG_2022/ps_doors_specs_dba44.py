...
cur = mydb.cursor()
cur.execute("SELECT product_url,attr_name,attr_value,id FROM `ps_doors_specs_dba44` where attr_test is null")
...
for fetch in myresult:
    ...
    if flag == 1:
        par = ("attribute found")
        sql = ("UPDATE `ps_doors_specs_dba44` SET attr_test ='" + str(par) + "' WHERE   id='" + str(id) + "'")
        cur.execute(sql)
        mydb.commit()
        ...
        sql = ("UPDATE `ps_doors_specs_dba44` SET attr_test ='" + str(par1) + "' WHERE   id='" + str(id) + "' ")
        cur.execute(sql)
        mydb.commit()
        print(cur.rowcount, "records successful Done")
