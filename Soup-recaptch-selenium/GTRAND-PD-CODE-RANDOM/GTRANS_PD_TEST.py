...
cur=mydb.cursor()
cur.execute("select product_url from `b2b_pd_test` where id>23566 and random_pd_test is null")
...
for fetch in myresults:
    ...
    if(a>0):
        y=('yes')
        sql = ("UPDATE `b2b_pd_test` SET random_pd_test ='" + str(y) + "' WHERE   product_url='" + str(url) + "'")
        cur.execute(sql)
        mydb.commit()
